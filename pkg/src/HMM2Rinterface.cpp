                             ...
                             SEXP seed)
{
    ...
    arma::mat d_graph = arma::conv_to<arma::mat>::from(ia_graph);
	...
	Gibbs_Sampling Runner(dataTest, na_temps, d_graph, ua_Emission, maxseqlength, 
	                        how_many_sequence_tries, iinternal, iswappings, randseed);


    //~ Rcpp::Rcout << "\nStart sampler ... "; Rcpp::Rcout.flush();	                        
	arma::cube runResult = Runner.run(iburn, imc);
	
	arma::cube runHashes = Runner.get_hash_trace();
	arma::cube runLikes = Runner.get_sequences_likelihood_trace();
	...
	arma::mat marginal_calculation_points = arma::zeros<arma::mat>(number_of_samples, runResult.n_cols);
	
	for (arma::uword i = 0; i < number_of_samples; i++)
	{
		arma::uword sampleIndex = indexlist[i];
		marginal_calculation_points(i,arma::span::all) = runResult.slice(0).row(sampleIndex);
	...
	double naiveMarlik = dataTest.naive_marginal_likelihood();
	...
	List HMMresult = List::create( _("temperature.indices") = Runner.get_temperature_trace(),
	                               _("mc.samples.transition.matrix") = runResult,
	                               _("mc.samples.hash.trace") = runHashes, 
	                               _("mc.samples.likelihood.trace") = runLikes,
	                               _("sorted.genotyping.data") = dataTest.get_genotype_list(),
	                               ...
	                               _("n.samples") = imc,
	                               _("jumping.probabilities") = Runner.get_temperature_likelihoods()
	                               );
